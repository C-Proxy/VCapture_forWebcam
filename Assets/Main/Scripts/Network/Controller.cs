...
public class Controller : MonoBehaviour
{
    ...
    [SerializeField]
    PointSphere m_LeftHandSphere, m_RightHandSphere, m_PoseSphere;
    [SerializeField]
    UdpReceiver m_UdpReceiver = default;
    [SerializeField]
    PoseRig m_PoseMechanim = default;

    PointSphere HandSphereScript;
    HandCylinder HandCylinderScript;
    // const string PROGRAM_PATH = "C:\\Users\\nc271\\WorkSpace\\Python\\WebcamCapture\\dist\\CaptureClient.exe";
    // PythonProgram PythonProgram = new PythonProgram("Python Program", "127.0.0.1", 8080, PROGRAM_PATH);
    ...
    void Start()
    {
        ...
        m_UdpReceiver.AssignCallback(ResponceEvents);
    }
    ...
    void ResponceEvents(string label, Vector3[] points)
    {
        switch (label)
        {
            case "Left":
                m_PoseMechanim.SetLeftHand(points);
                break;
            case "Right":
                m_PoseMechanim.SetRightHand(points);
                break;
            case "Pose":
                m_PoseMechanim.SetPose(points);
                break;
        }
